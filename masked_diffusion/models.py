...
class Attention(nn.Module):
    ...
    def get_masked_rel_bias(self, B, ids_keep):
        ...
        rel_pos_bias_masked = torch.gather(
            rel_pos_bias,
            dim=2,
            index=ids_keep.unsqueeze(dim=1)
            .unsqueeze(dim=-1)
            .repeat(1, rel_pos_bias.shape[1], 1, rel_pos_bias.shape[-1]),
        )
        rel_pos_bias_masked = torch.gather(
            rel_pos_bias_masked,
            dim=3,
            index=ids_keep.unsqueeze(dim=1)
            .unsqueeze(dim=2)
            .repeat(1, rel_pos_bias.shape[1], ids_keep.shape[1], 1),
        )
...
class LabelEmbedder(nn.Module):
    ...
    def token_drop(self, labels, force_drop_ids=None):
        ...
        labels = torch.where(drop_ids.to(labels.device), self.num_classes, labels)
        return labels
...
class MDTBlock(nn.Module):
    ...
    def __init__(
        self, hidden_size, num_heads, mlp_ratio=4.0, skip=False, **block_kwargs
    ):
        super().__init__()
...
class MDTv2(nn.Module):
    ...
    def __init__(
        ...
        if mask_ratio is not None:
            ...
            self.mask_token = nn.Parameter(
                torch.zeros(1, 1, hidden_size), requires_grad=False
            )
    ...
    def forward(self, x, t, y, enable_mask=False):
        ...
        if self.mask_ratio is not None and enable_mask:
            ...
            rand_mask_ratio = (
                rand_mask_ratio * 0.2 + self.mask_ratio
    ...
    def forward_with_cfg(
        self, x, t, y, cfg_scale=None, diffusion_steps=1000, scale_pow=4.0
    ):
        """
        ...
        if cfg_scale is not None:
            ...
            scale_step = (
                (1 - torch.cos(((1 - t / diffusion_steps) ** scale_pow) * math.pi))
                * 1
                / 2
            )  # power-cos scaling
